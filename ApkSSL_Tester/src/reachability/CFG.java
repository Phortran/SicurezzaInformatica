...
//		public int hashCode() { return (s==null)? 0 : StmtMapper.getWriteGlobalStmtIds().get(s); }
		...
		public static class NodeComparator implements Comparator<CFGNode> {
			...
			public static int _compare(CFGNode n1, CFGNode n2) {
				...
				CFG cfg1 = ProgramFlowGraph.getInstance().getContainingCFG(n1);
				CFG cfg2 = ProgramFlowGraph.getInstance().getContainingCFG(n2);
				if (cfg1 != cfg2)
					return (ProgramFlowGraph.getInstance().getMethodIdx(cfg1.getMethod()) < 
							ProgramFlowGraph.getInstance().getMethodIdx(cfg2.getMethod()))? -1 : 1;
	...
	private static class CFGComparator implements Comparator<CFG> {
		public int compare(CFG arg0, CFG arg1) {
			final int idx0 = ProgramFlowGraph.getInstance().getMethodIdx(arg0.method);
			final int idx1 = ProgramFlowGraph.getInstance().getMethodIdx(arg1.method);
	...
	public void analyze() {
		...
		for (CFGNode n : getNodes()) {
			...
			if (n.succs.size() > 1) {
				...
				//if (Options.removeRepeatedBranches()) { FIXME assumo sempre TRUE
...
//				Util.getConcreteCallTargets(invExpr, mAppTargets, mLibTargets);
				...
				if (cs != null) {
					for (SootMethod mAppCallee : cs.getAppCallees()) {
						CFG cfgCallee = ProgramFlowGraph.getInstance().getCFG(mAppCallee);
	...
	protected void createNodesPredsSuccs(SootMethod m) {
		...
		if (pchain.isEmpty()) {  // if no stmts, just link ENTRY and EXIT
			...
			for (Iterator<Unit> itStmt = pchain.iterator(); itStmt.hasNext(); ) {
				...
				if (s instanceof JRetStmt || s instanceof JReturnStmt || s instanceof JReturnVoidStmt) {
					n.addSucc(EXIT);
					EXIT.addPred(n);
				}
				else {
