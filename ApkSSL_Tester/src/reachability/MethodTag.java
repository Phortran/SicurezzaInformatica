...
public class MethodTag implements Tag {
	...
	public static class MethodComparator implements Comparator<SootMethod> {
		public int compare(SootMethod m1, SootMethod m2) {
			...
			return (ProgramFlowGraph.getInstance().getMethodIdx(m1) < 
						ProgramFlowGraph.getInstance().getMethodIdx(m2))? -1 : 1;
		}
	}
	...
	private boolean reachableFromEntry = false; //!Options.reachability(); FIXME l'analisi è abilitata di default
...
//		reachability = new ReachabilityAnalysis(m); // computes reachability right away
	...
	public void createStmtTags() {
		int sIdx = 0;
		for (Iterator<Unit> itStmt = m.retrieveActiveBody().getUnits().iterator(); itStmt.hasNext(); ) {
	...
	private void findPredsAndExits(boolean verbose) {
		...
		for (Iterator<Unit> itStmt = pchain.iterator(); itStmt.hasNext(); ) {
			...
			boolean isExit = s instanceof JRetStmt || s instanceof JReturnStmt || s instanceof JReturnVoidStmt;
	...
	private void findFormalParams() {
		...
		PatchingChain<Unit> pc = m.retrieveActiveBody().getUnits();
...
//			final boolean usingOrigNames = soot.options.Options.getDeclaredOptionsForPhase("jb").contains("enabled use-original-names");
		...
		Stmt s;
		for (Iterator<Unit> itS = pc.iterator(); itS.hasNext() && (s = (Stmt)itS.next()) instanceof IdentityStmt; ) {
...
//	}
	...
	private void setStmtBB(Block bb) {
		...
		for (Iterator<Block> itBBSucc = bb.getSuccs().iterator(); itBBSucc.hasNext(); ) {
...
//		PatchingChain pchain = m.retrieveActiveBody().getUnits();
	...
	public void removeRedundantBackReachCallSites() {
		for (CallSite cs : (HashSet<CallSite>)backwardReachedCallSites.clone()) {
			final int containingMtdId = ProgramFlowGraph.getInstance().getMethodIdx(cs.getLoc().getMethod());
