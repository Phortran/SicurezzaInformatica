...
import reachability.ProgramFlowGraph.EntryNotFoundException;
...
public class ReachabilityAnalysis {
	...
	public static void computeReachability(List<SootMethod> entryMethods) {
		...
		try {
			ProgramFlowGraph.createInstance(new CFGFactory());
		...
		List<SootMethod> indexedAppMethods = ProgramFlowGraph.getInstance().getReachableAppMethods();
		...
		for (SootMethod m : indexedAppMethods) {
			...
			for (CallSite csOut : mTag.getCallSites()) {
				...
				for (SootMethod mAppCallee :  csOut.getAppCallees())
					bsAppCallees.set(ProgramFlowGraph.getInstance().getMethodIdx(mAppCallee));
			...
			for (CallSite csIn : mTag.getCallerSites()) {
				...
				for (Stmt sCallSucc : sCallTag.getSuccessorStmts()) {
					...
					for (SootMethod mLocalFromCaller : sCallSuccTag.getLocalReachedAppMtds()) {
						...
						mTag.addBackwardReachedAppMtd(ProgramFlowGraph.getInstance().getMethodIdx(mLocalFromCaller));
	...
	private static void markReachableFromEntry(List<SootMethod> entryMethods) {
		...
		while (!entries.isEmpty()) {
			...
			for (SootMethod m : ProgramFlowGraph.getInstance().getReachableAppMethods()) {
				final int mId = ProgramFlowGraph.getInstance().getMethodIdx(m);
				if (m == entry || bsAllReachableFromEntry.get(mId)) {
					...
					if (!mTag.isReachableFromEntry()) {
						...
						assert ProgramFlowGraph.getInstance().getReachableAppMethods().contains(m);
						++reachCount;
		...
		assert reachCount <= ProgramFlowGraph.getInstance().getReachableAppMethods().size();
		int allUnits = 0;
		for (SootMethod m : ProgramFlowGraph.getInstance().getAppConcreteMethods())
			allUnits += m.retrieveActiveBody().getUnits().size();
		System.out.println("Methods reachable from entry: " + reachCount + "/" + ProgramFlowGraph.getInstance().getAppConcreteMethods().size()
	...
	public static boolean reachesFromTop(CFGNode nFrom, CFGNode nTo, boolean interproc) {
		...
		final int toMethodId = ProgramFlowGraph.getInstance().getMethodIdx(toMethod);
	...
	public static boolean reachesFromBottom(CFGNode nFrom, CFGNode nTo, boolean interproc) {
		...
		if (nFrom.getSuccs().isEmpty()) {
			...
			if (interproc && sFrom instanceof ReturnStmt) {
				SootMethod mCaller = ProgramFlowGraph.getInstance().getContainingMethod(nFrom);
				for (CallSite csIn : ((MethodTag)mCaller.getTag(MethodTag.TAG_NAME)).getCallerSites()) {
					if (!csIn.isReachableFromEntry() || csIn.isInCatchBlock())
						continue; // skip catch blocks, FOR NOW
					if (reachesFromBottom(ProgramFlowGraph.getInstance().getNode(csIn.getLoc().getStmt()), nTo, interproc))
						return true;
				}
			}
		}
	...
	public static boolean forwardReaches(SootMethod mFrom, SootMethod mTo) {
		...
		final int mToId = ProgramFlowGraph.getInstance().getMethodIdx(mTo);
